...
deep nw: bagging - pooling layer(удаление нейронов)

Att: вертикальные движения критичны, очень важны (?? может и нет)

fixme: looks like deep learning - elements of unsupervizing - абстракция на автомотическим выделением фич

fixme: странные и стремные промежуточные фотки лиц людей во внутренних слоях сети

fixme: deep learning - нужно **много данных** и **вычислительных мощностей/времени**

fixme: соотношение модель-данные - нейронка не большая, а огромная

... "Нужно изучить данные. Сперва методами обучения без учителя находим некоторые структуры, кластеры, затем уже учим с учителем, сперва простые модели, а затем усложняем" приближенно так из курсов по Big Data от EMC на Интуите
